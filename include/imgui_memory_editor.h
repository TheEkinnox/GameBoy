...
struct MemoryEditor
{
    ...
    void DrawOptionsLine(const Sizes& s, void* mem_data, size_t mem_size, size_t base_display_addr)
    {
        IM_UNUSED(mem_data);
        ImGuiStyle& style = ImGui::GetStyle();
        const char* format_range = OptUpperCaseHex ? "Range %0*" _PRISizeT "X..%0*" _PRISizeT "X | Addr:" : "Range %0*" _PRISizeT "x..%0*" _PRISizeT "x | Addr:"; // [CUSTOM_MOD] Add "Addr:" to range format

        // Options menu
        // [BEGIN CUSTOM_MOD] Hide Options button but keep range and current address
        // if (ImGui::Button("Options"))
        //     ImGui::OpenPopup("OptionsPopup");
        // ImGui::SameLine();

        const float cursorY = ImGui::GetCursorPosY();
        ImGui::SetCursorPosY(cursorY  + (ImGui::GetFrameHeightWithSpacing() - ImGui::CalcTextSize(format_range).y) * .5f);
        ImGui::Text(format_range, s.AddrDigitsCount, base_display_addr, s.AddrDigitsCount, base_display_addr + mem_size - 1);
        ImGui::SameLine(); ImGui::SetCursorPosY(cursorY + floorf(style.FramePadding.y * .5f));
        // [END CUSTOM_MOD]
    ...
    void DrawPreviewLine(const Sizes& s, void* mem_data_void, size_t mem_size, size_t base_display_addr)
    {
        ...
        ImGui::Text("View as:"); // [CUSTOM_MOD] Replace "Preview as" with "View as" (makes the window slightly smaller)
        ImGui::SameLine();
        ImGui::SetNextItemWidth((s.GlyphWidth * 10.0f) + style.FramePadding.x * 2.0f + style.ItemInnerSpacing.x);

        // [CUSTOM_MOD] Make supported ImGuiDataType array constexpr and limit to Uint16
        static constexpr ImGuiDataType supported_data_types[] = { ImGuiDataType_S8, ImGuiDataType_U8, ImGuiDataType_S16, ImGuiDataType_U16 };
    ...
    {
        // [CUSTOM_MOD] Make ImGuiDataType Descs array static constexpr and limit to Uint16
        static constexpr const char* descs[] = { "Int8", "Uint8", "Int16", "Uint16" };
